...
tot⟦tm⟧'{l}{C = C}{T} p q =
  let
    ...
    e' : 𝒞 ⋉ ℱ𝒶𝓂 l ⋉ ℰ𝓁ℯ𝓂 l ∋ ((C' , T') , t') ~ ((C , T) , t)
    e' = (e , coh (ℰ𝓁ℯ𝓂 l){C' , T'}{C , T} e t')
  in
  (t , resp⟦tm⟧ q'' e')
...
sound⟦tm⟧'{l}{C = C}{T} p q =
  let
    (C' , T' , t' , q₀ , q₁ , q₂) = sound⟦tm⟧ p
...
tot⟦tm⟧ (⊢𝐬𝐮𝐜𝐜 p) =
  let
    ...
    t : Elem 0 C 𝒩𝒶𝓉
    t = coe (ℰ𝓁ℯ𝓂 0) (sv⟦ty⟧ q (⟦𝐍𝐚𝐭⟧ (ok⟦ty⟧ q))) t'

    e : 𝒞 ⋉ ℱ𝒶𝓂 0 ⋉ ℰ𝓁ℯ𝓂 0 ∋ ((C , T) , t') ~ ((C , 𝒩𝒶𝓉)  , t)
    e = (sv⟦ty⟧ q (⟦𝐍𝐚𝐭⟧ (ok⟦ty⟧ q)) ,
      coh (ℰ𝓁ℯ𝓂 0) (sv⟦ty⟧ q (⟦𝐍𝐚𝐭⟧ (ok⟦ty⟧ q))) t')
  in
...
tot⟦tm⟧{Γ = Γ} (⊢𝐧𝐫𝐞𝐜{l}{B}{b₀}{a}{b₊} X q₀ q₁ q₂ h)
  ...
  q' : ⟦ Γ ⨟ x ∶[ 0 ] 𝐍𝐚𝐭 ⨟ y ∶[ l ] B [ x ] ⊢[ l ]
    B [ 𝐬𝐮𝐜𝐜 (𝐯 x) ] ⟧ty↘(C ⋉[ 0 ] 𝒩𝒶𝓉 ⋉[ l ] S , 𝓅 S * T)
  q' = wk⟦ty⟧
    qS
    (subst (λ B' → ⟦ Γ ⨟ x ∶[ 0 ] 𝐍𝐚𝐭 ⊢[ l ] B' ⟧ty↘(C ⋉[ 0 ] 𝒩𝒶𝓉 , T))
...
sound⟦tm⟧{Γ = Γ} (𝚷Cong{l}{l'}{B = B}{B'} X q₀ q₁ h)
  with (x , x#X ∉∪ x#B ∉∪ x#B' ∉∪ x#Γ) ← fresh (X , B , B' , Γ) =
  let
    (C , S , q) = tot⟦ty⟧ h
    (S' , qS , qS') = sound⟦ty⟧' q₀ (ok⟦ty⟧ q)
    (T , qT , qT') = sound⟦ty⟧' (q₁ x x#X) (⟦⨟⟧ q x#Γ)
...
sound⟦tm⟧{Γ = Γ} (𝛌Cong{l}{l'}{B = B}{b}{b'} X q₀ q₁ h₀ h₁)
...
sound⟦tm⟧{Γ = Γ} (∙Cong{l}{l'}{B = B}{B'} X q₀ q₁ q₂ q₃ h₀ h₁)
  with (x , x#X ∉∪ x#B ∉∪ x#B' ∉∪ x#Γ) ← fresh (X , B , B' , Γ) =
  let
    (C , S , qS , qS') = sound⟦ty⟧ q₀
    (T , qT  , qT') = sound⟦ty⟧' (q₁ x x#X) (⟦⨟⟧ qS x#Γ)
    (s , qs , qs') = sound⟦tm⟧' q₃ qS
    (t , qt , qt') = sound⟦tm⟧' q₂ (⟦𝚷⟧⁻ qS qT x#B)
  in
  (C , ⟪ s ⟫ * T , 𝒶𝓅𝓅 l l' S T t s ,
  ⟦conc⟧ B x qT qs x#B ,
  ⟦∙⟧⁻ qt qS qT qs x#B ,
...
sound⟦tm⟧ (𝐄𝐪Cong{l} q₀ q₁ q₂) =
  let
    (C , T , qT , qT') = sound⟦ty⟧ q₀
    (s , qs , qs') = sound⟦tm⟧' q₁ qT
    (t , qt , qt') = sound⟦tm⟧' q₂ qT
  in
  (C , 𝓊𝓃𝒾𝓋 l , ℰ𝓆 l T s t ,
    ⟦𝐔⟧ (ok⟦ty⟧ qT) ,
    ⟦𝐄𝐪⟧ qT qs qt ,
...
sound⟦tm⟧ (𝐞𝐦𝐩Cong{l} q₀ q₁) =
  let
    (C , T , qT , qT') = sound⟦ty⟧ q₀
    (e , qe , qe') = sound⟦tm⟧' q₁ (⟦𝐄𝐦𝐩⟧ (ok⟦ty⟧ qT))
  in
  (C , T , ℯ𝓂𝓅 l T e , qT , ⟦𝐞𝐦𝐩⟧ qT qe , ⟦𝐞𝐦𝐩⟧ qT' qe')

sound⟦tm⟧ (𝐫𝐞𝐟𝐥Cong{l} q₀ q₁) =
  let
    (C , T , qT , qT') = sound⟦ty⟧ q₀
    (t , qt , qt') = sound⟦tm⟧' q₁ qT
  in
...
sound⟦tm⟧{Γ = Γ} (𝐧𝐫𝐞𝐜Cong{l}{C = B}{B'}{b₀}{b₀'}{a}{a'}{b₊}{b₊'}
  X q₀ q₁ q₂ q₃ h)
  ...
  r₀ :  ∑[ t ∈ Elem l C (⟪ 𝓏ℯ𝓇ℴ ⟫ * S) ]
        ⟦ Γ ⊢[ l ] b₀ ⟧tm↘((C , ⟪ 𝓏ℯ𝓇ℴ ⟫ * S) , t) ∧
        ⟦ Γ ⊢[ l ] b₀' ⟧tm↘((C , ⟪ 𝓏ℯ𝓇ℴ ⟫ * S) , t)
  r₀ = sound⟦tm⟧' q₁ (⟦conc⟧ B x qS (⟦𝐳𝐞𝐫𝐨⟧ q) x#B)
  s₀ = π₁ r₀
  qs₀ = π₁ (π₂ r₀)
  qs₀' = π₂ (π₂ r₀)
  T : Fam l (C ⋉[ 0 ] 𝒩𝒶𝓉)
  ...
  q' = wk⟦ty⟧
    ...
    (subst (λ B' → ⟦ Γ ⨟ x ∶[ 0 ] 𝐍𝐚𝐭 ⊢[ l ] B' ⟧ty↘(C ⋉[ 0 ] 𝒩𝒶𝓉 , T))
      ...
      (sb⟦ty⟧
        ...
        qS))
    (y#Γ ∉∪ (#symm x#y))
  r₊ : ∑[ t ∈ (Elem l (C ⋉[ 0 ] 𝒩𝒶𝓉 ⋉[ l ] S) (𝓅 S * T)) ]
       ⟦ Γ ⨟ x ∶[ 0 ] 𝐍𝐚𝐭 ⨟ y ∶[ l ] B [ x ] ⊢[ l ] b₊ [ x ][ y ] ⟧tm↘
       ((C ⋉[ 0 ] 𝒩𝒶𝓉 ⋉[ l ] S , 𝓅 S * T) , t)
       ∧
       ⟦ Γ ⨟ x ∶[ 0 ] 𝐍𝐚𝐭 ⨟ y ∶[ l ] B [ x ] ⊢[ l ] b₊' [ x ][ y ] ⟧tm↘
       ((C ⋉[ 0 ] 𝒩𝒶𝓉 ⋉[ l ] S , 𝓅 S * T) , t)
  r₊ = sound⟦tm⟧' (q₂ x y (##:: y#X (##:: (x#y ∉∪ x#X) ##◇))) q'
  s₊ = π₁ r₊
  qs₊ = π₁ (π₂ r₊)
  qs₊' : ⟦ Γ ⨟ x ∶[ 0 ] 𝐍𝐚𝐭 ⨟ y ∶[ l ] B' [ x ] ⊢[ l ] b₊' [ x ][ y ] ⟧tm↘
    ((C ⋉[ 0 ] 𝒩𝒶𝓉 ⋉[ l ] S , 𝓅 S * T) , s₊)
  qs₊' = sound⟦cx⟧ qS qS' (π₂ (π₂ r₊))

sound⟦tm⟧{Γ = Γ} (𝚷Beta{l}{l'}{A}{a}{B}{b} X q₀ q₁ h₀ h₁)
  with (x , x#X ∉∪ x#B ∉∪ x#b ∉∪ x#Γ) ← fresh (X , B , b , Γ) =
  let
...
sound⟦tm⟧{Γ = Γ} (𝐍𝐚𝐭Beta₀{l}{B}{b₀}{b₊} X q₀ q₁ h)
  with (y , y#X ∉∪ y#b₊ ∉∪ y#Γ) ← fresh (X , b₊ , Γ)
...
sound⟦tm⟧{Γ = Γ} (𝐍𝐚𝐭Beta₊{l}{B}{b₀}{a}{b₊} X q₀ q₁ q₂ h)
  with (y , y#X ∉∪ y#b₊ ∉∪ y#Γ) ← fresh (X , b₊ , Γ)
  with (x , x#y ∉∪ x#X ∉∪ x#B ∉∪ x#b₀ ∉∪ x#b₊ ∉∪ x#Γ) ←
    fresh (y , X , B , b₀ , b₊ , Γ) =
  ...
  q'' = resp⟦tm⟧
    ...
    ((rflᶜ C ,
     hrfl (ℱ𝒶𝓂 l) C (⟪ s ⟫ * T)) ,
     hrfl (ℰ𝓁ℯ𝓂 l) (C , ⟪ s ⟫ * T) (𝓃𝓇ℯ𝒸 l S s₀ s₊ (𝓈𝓊𝒸𝒸 s)))

sound⟦tm⟧ (Reflect{l} q₀ q₁ q₂ h) =
  let
    (C , T , qT) = tot⟦ty⟧ h
    (t , qt) = tot⟦tm⟧' q₀ qT
    (t' , qt') = tot⟦tm⟧' q₁ qT
...
sound⟦tm⟧ (UIP{l} q₀ q₁ q₂ q₃ h) =
  ...
  (C , ℰ𝓆 l T t t' , e , ⟦𝐄𝐪⟧ qT qt qt' , qe , resp⟦tm⟧ qe'
    ((rflᶜ C , hrfl (ℱ𝒶𝓂 l) C (ℰ𝓆 l T t t')) ,
      hsym (ℰ𝓁ℯ𝓂 l){y = e}{e'}
        (rfl (𝒞 ⋉ ℱ𝒶𝓂 l) (C , ℰ𝓆 l T t t'))
        (𝓊𝒾𝓅 l T t t' e e')))

sound⟦tm⟧{Γ = Γ} (𝚷Eta{l}{l'}{A}{B}{b}{b'} X q₀ q₁ q₂ h₀ h₁)
  with (x , x#X ∉∪ x#B ∉∪ x#Γ) ← fresh (X , B , Γ) =
  ...
  T : Fam l' (C ⋉[ l ] S)
  T = π₁ rh₁
  qT : ⟦ Γ ⨟ x ∶[ l ] A ⊢[ l' ] B [ x ] ⟧ty↘(C ⋉[ l ] S , T)
  qT = π₂ rh₁

  r₀ : ∑[ t₀ ∈ Elem (max l l') C (𝒫𝒾 l l' S T) ]
  ...
  re : ∑[ te ∈ Elem l' (C  ⋉[ l ] S) T ]
       (⟦ Γ ⨟ x ∶[ l ] A ⊢[ l' ] b ∙[ A , B ] 𝐯 x ⟧tm↘
         ((C ⋉[ l ] S , T) , te))
       ∧
       (⟦ Γ ⨟ x ∶[ l ] A ⊢[ l' ] b' ∙[ A , B ] 𝐯 x ⟧tm↘
         ((C ⋉[ l ] S , T) , te))
  re = sound⟦tm⟧' (q₂ x x#X) qT
  te : Elem l' (C  ⋉[ l ] S) T
  te = π₁ re
  qe₀ : ⟦ Γ ⨟ x ∶[ l ] A ⊢[ l' ] b ∙[ A , B ] 𝐯 x ⟧tm↘
    ((C ⋉[ l ] S , T) , te)
  qe₀ = π₁ (π₂ re)
  qe₁ : ⟦ Γ ⨟ x ∶[ l ] A ⊢[ l' ] b' ∙[ A , B ] 𝐯 x ⟧tm↘
    ((C ⋉[ l ] S , T) , te)
  ...
  t₀' : Elem (max l l') (C ⋉[ l ] S) (𝒫𝒾 l l' S' T')
  t₀' = coe (ℰ𝓁ℯ𝓂 (max l l'))
  ...
  t₁₀ : (ℰ𝓁ℯ𝓂 (max l l')) ′ (C , 𝒫𝒾 l l' S T) ∋  t₁ ~ t₀
...
-- sound⟦cx⟧ ↘◇ = (Unit , ⟦◇⟧ , ⟦◇⟧)
-- sound⟦cx⟧ (↘⨟{l} q₀ q₁ (q₂ ∉∪ q₂') h₀ h₁) =
--   let
--     (C , q , q') = sound⟦cx⟧ q₀
--     (T , r , r') = sound⟦ty⟧' q₁ q
--     (T' , s , s') = sound⟦ty⟧' (↘⊢ q₁ (CxSymm q₀)) q'
--   in {!!}
--   --
--   --   (T' , q'') = tot⟦ty⟧' (↘⊢ h₁ q₀) q
--   --   (T'' , q''') = tot⟦ty⟧' h₁ q'
