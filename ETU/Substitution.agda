...
⊢idˢ (ok⨟{l}{Γ}{A}{x} q q' q'') = ⨟ˢ
  (▷Sb _ q (⊢idˢ q'') q')
  q
  (⊢𝐯 ([]⁻ q q') (subst (λ A' → (x , A' , l) isIn (Γ ⨟ x ∶[ l ] A))
    (symm (sbUnit A))
    isInNew))
  q'
...
sbJg{σ}{Δ} p (⊢𝐧𝐫𝐞𝐜{l}{C}{c₀}{a}{c₊} S q₀ q₁ q₂ h)
  ...
  module _ (x : 𝔸)(x#S : x # S)(x#Δ : x # Δ)(x#C : x # C)(x#c₊ : x # c₊)
    ...
    module _
      ...
      q₁' : (Δ ⨟ x ∶[ 0 ] 𝐍𝐚𝐭 ⨟ y ∶[ l ] (σ * C)[ x ]) ⊢
        (σ * c₊)[ x ][ y ] ∶[ l ] (σ * C) [ 𝐬𝐮𝐜𝐜 (𝐯 x) ]
      q₁' = subst₂ (λ c' C' →
        (Δ ⨟ x ∶[ 0 ] 𝐍𝐚𝐭 ⨟ y ∶[ l ] (σ * C)[ x ]) ⊢ c' ∶[ l ] C')
...
sbJg{σ}{Δ} p (𝛌Cong{l}{l'}{A}{A'}{B}{b}{b'} S q₀ q₁ h₀ h₁) = 𝛌Cong
  ...
  (λ{x (x#S ∉∪ x#Δ ∉∪ x#B ∉∪ x#b ∉∪ x#b') →
    subst₃ (λ c c' C → (Δ ⨟ x ∶[ l ] σ * A) ⊢ c ＝ c' ∶[ l' ] C)
      (sbUpdate[] σ x (𝐯 x) b x#b)
      (sbUpdate[] σ x (𝐯 x) b' x#b')
      (sbUpdate[] σ x (𝐯 x) B x#B)
      (sbJg
...
sbJg{σ}{Δ} p (𝐧𝐫𝐞𝐜Cong{l}{C}{C'}{c₀}{c₀'}{a}{a'}{c₊}{c₊'}
  ...
  module _ (x : 𝔸)(x#S : x # S)(x#Δ : x # Δ)
    ...
    module _
      ...
      q₂' : (Δ ⨟ x ∶[ 0 ] 𝐍𝐚𝐭 ⨟ y ∶[ l ] (σ * C)[ x ]) ⊢
        (σ * c₊)[ x ][ y ] ＝ (σ * c₊')[ x ][ y ] ∶[ l ] (σ * C) [ 𝐬𝐮𝐜𝐜 (𝐯 x) ]
      q₂' = subst₃ (λ d d' D →
        (Δ ⨟ x ∶[ 0 ] 𝐍𝐚𝐭 ⨟ y ∶[ l ] (σ * C) [ x ]) ⊢ d ＝ d' ∶[ l ] D)
        (sbUpdate[]² σ x y (𝐯 x) (𝐯 y) c₊ x#c₊ (y#c₊ ∉∪ #symm x#y))
        (sbUpdate[]² σ x y (𝐯 x) (𝐯 y) c₊' x#c₊' (y#c₊' ∉∪ #symm x#y))
        eq'
        (sbJg
...
sbJg{σ}{Δ} p (𝚷Eta{l}{l'}{A}{B}{b}{b'} S q₀ q₁ q₂ h₀ h₁) = 𝚷Eta
  (S ∪ supp Δ)
  (sbJg p q₀)
  (sbJg p q₁)
  (λ{x (x#S ∉∪ x#Δ) → q₂' x x#S x#Δ})
  (sbJg p h₀)
  λ{x (x#S ∉∪ x#Δ) → h₁' x x#S x#Δ}
  where
  module _ (x : 𝔸)(x#S : x # S)(x#Δ : x # Δ) where
    ...
    q₂' : (Δ ⨟ x ∶[ l ] σ * A) ⊢
      (σ * b)  ∙[ σ * A , σ * B ] (𝐯 x) ＝
      (σ * b') ∙[ σ * A , σ * B ] (𝐯 x) ∶[ l' ] (σ * B) [ x ]
    q₂' = subst₃ (λ c c' C → (Δ ⨟ x ∶[ l ] σ * A) ⊢ c ＝ c' ∶[ l' ] C)
      (eq b x#b)
      (eq b' x#b')
      (sbUpdate[] σ x (𝐯 x) B x#B)
      (sbJg
...
sbJg p (Reflect q₀ q₁ q₂ h) = Reflect
  (sbJg p q₀)
  (sbJg p q₁)
  (sbJg p q₂)
  (sbJg p h)

sbJg p (UIP q₀ q₁ q₂ q₃ h) = UIP
  (sbJg p q₀)
  (sbJg p q₁)
  (sbJg p q₂)
  (sbJg p q₃)
...
ssbUpdate{l}{Γ}{A}{a} q x#Γ h = sbUpdate
  (⊢idˢ (⊢ok q))
  (subst (λ A' → Γ ⊢ a ∶[ l ] A') (symm (sbUnit A)) q)
  x#Γ
  h
...
ssb＝Update{l}{Γ}{A}{a}{a'} q x#Γ h = sb＝Update
  (sb＝Refl (⊢idˢ (⊢ok q)))
  (subst (λ A' → Γ ⊢ a ＝ a' ∶[ l ] A') (symm (sbUnit A)) q)
  x#Γ
  h
...
lift＝Sb{l}{σ}{σ'}{Γ}{Γ'}{A}{x}{x'} p q x#Γ x'#Γ h =
  ...
  q'' : (Γ' ⨟ x' ∶[ l ] σ * A) ⊢
      (σ ∘/ x := 𝐯 x') x ＝ (σ' ∘/ x := 𝐯 x') x ∶[ l ] (σ ∘/ x := 𝐯 x') * A
...
rn⨟²{Γ}{x}{x'}{y}{y'}{A}{B}{l}{l'} q x'#Γ (y'#x' ∉∪ y'#Γ)
  ...
  sbJg p' q
  where
  p : Γ ⨟ x' ∶[ l ] A ⊢ˢ x := 𝐯 x' ∶ Γ ⨟ x ∶[ l ] A
  p = subst (λ A' →
    Γ ⨟ x' ∶[ l ] A' ⊢ˢ x := 𝐯 x' ∶ Γ ⨟ x ∶[ l ] A)
    (sbUnit A)
    (liftSb⁻ (⊢idˢ okΓ) q'' x#Γ x'#Γ)

  p' : (Γ ⨟ x' ∶[ l ] A ⨟ y' ∶[ l' ] (x := 𝐯 x') * B) ⊢ˢ
    (x := 𝐯 x' ∘/ y := 𝐯 y') ∶ (Γ ⨟ x ∶[ l ] A ⨟ y ∶[ l' ] B)
  p' = liftSb²
    (⊢idˢ okΓ) q'' q'
